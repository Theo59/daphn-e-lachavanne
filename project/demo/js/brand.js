// Brand kit — demo interactif
// Copie de project/js/brand.js avec chemins médias ajustés (../media/)
...
// GradientBlob
function GradientBlob({ variant: variantProp, style = {}, animated = true } = {}) {
  ...
  return `<div style="${wrapStyle}"><svg width="100%" height="100%" viewBox="0 0 600 800" preserveAspectRatio="xMidYMid slice" style="display:block"><defs><radialGradient id="${id}-rg" cx="50%" cy="50%" r="65%">${stopTags}</radialGradient><filter id="${id}-bl" x="-30%" y="-30%" width="160%" height="160%"><feGaussianBlur stdDeviation="${p.blur}"/></filter></defs><g filter="url(#${id}-bl)"><ellipse cx="300" cy="400" rx="320" ry="380" fill="url(#${id}-rg)">${animTags}</ellipse></g></svg></div>`;
}
...
function Sym({ kind, size = 48, stroke = 1.2, color = 'currentColor' } = {}) {
  ...
  switch (kind) {
    case 'breath':   return `<svg ${a}><ellipse cx="24" cy="24" rx="20" ry="10"/><ellipse cx="24" cy="24" rx="14" ry="6"/><circle cx="24" cy="24" r="1.5" fill="${color}"/></svg>`;
    case 'spiral':   return `<svg ${a}><path d="M24 24 m-2 0 a2 2 0 1 1 4 0 a4 4 0 1 1 -8 0 a6 6 0 1 1 12 0 a8 8 0 1 1 -16 0 a10 10 0 1 1 20 0 a12 12 0 1 1 -24 0"/></svg>`;
    case 'sun': {
      const lines = [...Array(8)].map((_, i) => {
        const ang = (i * Math.PI) / 4;
        return `<line x1="${(24+Math.cos(ang)*10).toFixed(2)}" y1="${(24+Math.sin(ang)*10).toFixed(2)}" x2="${(24+Math.cos(ang)*15).toFixed(2)}" y2="${(24+Math.sin(ang)*15).toFixed(2)}"/>`;
      }).join('');
      return `<svg ${a}><circle cx="24" cy="24" r="6"/><g>${lines}</g></svg>`;
    }
    case 'wave':     return `<svg ${a}><path d="M4 28 Q12 18 20 28 T36 28 T52 28"/><path d="M4 36 Q12 26 20 36 T36 36 T52 36" opacity="0.5"/></svg>`;
    case 'triangle': return `<svg ${a}><path d="M24 8 L40 38 L8 38 Z"/></svg>`;
    case 'dot':      return `<svg ${a}><circle cx="24" cy="24" r="2" fill="${color}"/><circle cx="24" cy="24" r="10"/><circle cx="24" cy="24" r="18" opacity="0.5"/></svg>`;
    case 'arrow':    return `<svg ${a}><line x1="6" y1="24" x2="42" y2="24"/><path d="M34 16 L42 24 L34 32"/></svg>`;
    case 'plus':     return `<svg ${a}><line x1="24" y1="10" x2="24" y2="38"/><line x1="10" y1="24" x2="38" y2="24"/></svg>`;
    default: return '';
  }
}

// Photo
function Photo({ src, alt = '', aspect = '4/5', filter = '', style = {} } = {}) {
  ...
  return `<div class="photo-wrap" style="${wrapStyle}"><img src="${src}" alt="${alt}" style="width:100%;height:100%;object-fit:cover;${f}display:block"/></div>`;
}
...
function PhotoSlot({ label = 'Photo', kind = 'photo', aspect = '4/5', style = {} } = {}) {
  ...
  const wrapStyle = st({
    ...
    borderRadius: 2, overflow: 'hidden', display: 'flex', alignItems: 'flex-end', padding: 14, ...style,
  });
  const badge = isVideo ? `<div style="position:absolute;top:14px;right:14px;display:flex;align-items:center;gap:6px;color:rgba(255,255,255,0.85);font-family:'Tenor Sans',serif;font-size:10px;letter-spacing:0.18em;text-transform:uppercase"><span style="width:6px;height:6px;border-radius:50%;background:#ff7100;box-shadow:0 0 8px #ff7100"></span>vidéo</div>` : '';

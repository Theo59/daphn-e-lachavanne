...
3. **Term/topic gap**:
   ...
   about X" to get a qualitative missing-topics list — reading beats pure term frequency for judging
...
5. **Backlinks**: our own site only, via Google Search Console → Links — no reliable free source for
...
1. **Structured data check**: `curl <url> -s | grep -o '<script type="application/ld+json"[^>]*>.*'`
   (or write a small parse step) — read what JSON-LD is ACTUALLY on the page. This project already
   emits `LocalBusiness`/`Person`/`WebSite`/`Service`/`FAQPage`/`BreadcrumbList` (see
   `src/layouts/Layout.astro` + `src/lib/schema.ts`) — check what's genuinely missing before adding
   more, rather than assuming a gap.
...
7. Ship fixes the way this project always has: edit content in **Sanity directly** (immediate live
   ...
   for fallback parity — every content fix goes through both, on a branch + PR (never main).
8. Note what you found and fixed (score before/after, what changed) so the next pass can compare.
...
- No backlink index for competitors — only our own site, via Google Search Console.
- `page-metrics.mjs` timing is a single script-side fetch, not a real-browser Lighthouse run.
- Any computed or fetched metric can be stale or wrong (a cached page, a crawler that hasn't run
  recently, a schema check that misses something that's actually there). Cross-check against the
  live page/code before writing a fix — never act on a single number without verifying it first.
